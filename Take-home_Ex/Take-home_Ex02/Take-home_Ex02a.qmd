...
2.  `sf`: for importing, managing, and processing geospatial data

3.  `tidyverse`: a family of other R packages for performing data science tasks such as importing, wrangling, and visualizing data

4.  `tmap`: creating static and interactive maps

5.  `ggplot2`: used for data visualization

6.  `lubridate`: for working with date-time data

Pacman assists us by helping us load R packages that we require.

```{r}
pacman::p_load(dplyr,maptools, rgdal,sf, sp, tidyverse, tmap, ggplot2, plotly, lubridate, gifski)
...
Filter for cases in Tainan only.
...
```{r}
dengue_clean <- dengue %>%
  filter(week >= 31 & week <= 50 & year(發病日) == 2023)
```
...
#| echo: false
#| results: hide
dengue_week <- tm_shape(dengue_join) +
  tm_fill("case_count", palette = "Reds") +
  tm_borders(lwd = 0.1) +
...
Let's visualise the distribution of dengue cases in various villages across the weeks.

```{r}
tmap_animation(dengue_week, filename = "dengue_week.gif", delay = 150, width = 800, height = 600, scale = 1.5)

```

```{r}
knitr::include_graphics("dengue_week.gif")
```
