...
```{r}
pacman::p_load(sf, sfdep, tidyverse, tmap, spdep, knitr, leaflet, ggthemes, knitr, plotly, igraph, ggraph, corrplot, data.table, reshape2, heatmaply, stars)
```
...
Unable to select a range for time, only single time record

Each bus stop has a different start-stop hour

```{r}

generate_trip_origin_weekday <- function(data, origin_pt_code) {
  origin_data <- data %>%
    filter(ORIGIN_PT_CODE == origin_pt_code & DAY_TYPE == "WEEKDAY") %>%
  st_as_sf(coords = c("ORIGIN_LAT", "ORIGIN_LONG"), crs = 4326)
  if (nrow(origin_data) == 0) {
    warning("No data found for the specified origin point code.")
    return(NULL)
  }
  
  min_time <- min(origin_data$TIME_PER_HOUR)
  max_time <- max(origin_data$TIME_PER_HOUR)
  
  trips_generated <- origin_data %>%
    filter(TIME_PER_HOUR >= min_time & TIME_PER_HOUR <= max_time) %>%
...
origin_pt_code <- 18159  
nov_trip_generation_origin2 <- generate_trip_origin(merged_bus_nov, origin_pt_code)
print(nov_trip_generation_origin2)
...
#   filter(TIME_PER_HOUR >= 6 & TIME_PER_HOUR <= 20) %>% #single hour selection
...
```{r}
nov_org_tg_spacetime <- as_spacetime(nov_trip_generation_origin2, 
                    "ORIGIN_PT_CODE", 
                    "TIME_PER_HOUR")
...
```{r}
nb <- st_knn(nov_trip_generation_origin2, 
             k = 1, symmetric = FALSE)
head (nb)
```
...
```{r}
ehsa_weekday <- nov_trip_generation_origin2 %>%
  cbind(gi_stars)
```

```{r}
bus_18159 <- ehsa_weekday %>%
  filter(ORIGIN_PT_CODE == "18159") %>%
  select(ORIGIN_PT_CODE, TIME_PER_HOUR, gi_star)
```

```{r}
ggplot(data =  bus_18159,
       aes(x = TIME_PER_HOUR,
           y = gi_star)) + 
...
Checking which bus stop codes have missing time_per_hours from 6-23, over 500 records.

```{r}
# Filtered data with time_per_hour between 6 and 23
filtered_data <- nov_trip_generation_origin 

# All unique origin PT codes in the dataset
all_origin_pt_codes <- filtered_data %>%
  distinct(ORIGIN_PT_CODE)

# Total number of hours from 6 to 23
total_hours <- 18  # 23 - 6 + 1

# Function to check if all hours are present for each origin PT code
check_all_hours_present <- function(data) {
  data %>%
    count(TIME_PER_HOUR) %>%
    summarise(all_hours_present = n() == total_hours)
}

# Check for each origin PT code
result <- filtered_data %>%
  group_by(ORIGIN_PT_CODE) %>%
  do(check_all_hours_present(.))

# Filter out only the ORIGIN_PT_CODEs with missing hours
missing_origin_pt_codes <- result %>%
  filter(!all_hours_present)

print(missing_origin_pt_codes)


```

```{r}
#write_rds(missing_origin_pt_codes, "data/rds/missing_origin_pt_codes.rds")
```

Checking which hour exactly is missing

```{r}
filtered_data <- nov_trip_generation_origin %>%
  filter(ORIGIN_PT_CODE == 18159, TIME_PER_HOUR >= 6 & TIME_PER_HOUR <= 23)

# Count occurrences of each TIME_PER_HOUR
time_per_hour_count <- filtered_data %>%
  count(TIME_PER_HOUR)

total_count <- sum(time_per_hour_count$n)

print(total_count)
```

```{r}
filtered_bus_nov <- bus_nov %>%
  filter(ORIGIN_PT_CODE == 18159)
```
